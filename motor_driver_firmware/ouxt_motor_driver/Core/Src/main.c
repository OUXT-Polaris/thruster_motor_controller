...
int main(void) {
  ...
  motorSpeedSemaphoreHandle =
      osSemaphoreCreate(osSemaphore(motorSpeedSemaphore), 1);
  ...
  motorTImerHandle =
      osTimerCreate(osTimer(motorTImer), osTimerPeriodic, &motor);
...
static void MX_TIM1_Init(void) {
  ...
  sBreakDeadTimeConfig.DeadTime = 0xac;
...
void StartDefaultTask(void const *argument) {
  ...
  /* USER CODE BEGIN 5 */
  for (;;) {
    osSemaphoreWait(motorSpeedSemaphoreHandle, osWaitForever);
    ros_data = GetROSData();
    memcpy(&f7_data, &ros_data, sizeof(struct send_data));
    osSemaphoreRelease(motorSpeedSemaphoreHandle);
    SendF7Data(&f7_data);
    osDelay(10);
  }
  /* USER CODE END 5 */
...
void motorCallback(void const *motor_ptr) {
  /* USER CODE BEGIN motorCallback */
  osSemaphoreWait(motorSpeedSemaphoreHandle, osWaitForever);
  motorSetSpeed((MotorHandleType *)motor_ptr, ros_data.motor_speed);
  osSemaphoreRelease(motorSpeedSemaphoreHandle);
